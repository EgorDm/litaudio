...
fn mock_container() -> AudioDeinterleaved<f64, U3, Dynamic> {
	AudioDeinterleaved::from_vec(U3, Dynamic::new(2), vec![1., 2., 3., 4., 5., 6.])
}

#[test]
fn iter() {
	let mut s = mock_container();
	assert_eq!(s.as_channel_slice_iter(1).cloned().collect::<Vec<_>>(), vec![3., 4.]);
	assert_eq!(s.as_channel_slice_mut_iter(1).map(|x| *x).collect::<Vec<_>>(), vec![3., 4.]);
	assert_eq!(s.slice_channels(1..3).as_channel_slice_iter(0).cloned().collect::<Vec<_>>(), vec![3., 4.]);
	assert_eq!(s.slice_samples(1).as_channel_slice_iter(1).cloned().collect::<Vec<_>>(), vec![4.]);

	assert_eq!(s.as_sample_slice_iter(1).cloned().collect::<Vec<_>>(), vec![2., 4., 6.]);
	assert_eq!(s.as_sample_slice_mut_iter(1).map(|x| *x).collect::<Vec<_>>(), vec![2., 4., 6.]);
	assert_eq!(s.slice_channels(1..3).as_sample_slice_iter(1).cloned().collect::<Vec<_>>(), vec![4., 6.]);
	assert_eq!(s.slice_samples(1).as_sample_slice_iter(0).cloned().collect::<Vec<_>>(), vec![2., 4., 6.]);

	assert_eq!(s.as_channel_slice_iter(2).cloned().collect::<Vec<_>>(), vec![5., 6.]);
...
#[test]
fn ops() {
	let s = AudioDeinterleaved::from_vec(U3, Dynamic::new(2), vec![1., 2., 3., 4., 5., 6.]);
	let s1 = AudioInterleaved::from_vec(U3, Dynamic::new(2), vec![1., 2., 3., 4., 5., 6.]);
